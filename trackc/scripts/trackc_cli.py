...
def _make_spec(conf):
    ...
    axs = pd.DataFrame({'rank_u': range(len(df['ax'].unique()))}, index=df['ax'].unique())
    axs['height'] = 1
    axs['hspace'] = 0.05
    axs['bottom'] = 'bottom'

    for i, tt in enumerate(conf["trackc"]):
        height, hspace = init_ax(tt, axs.loc[tt['ax'], 'height'], axs.loc[tt['ax'], 'hspace'])
...
def hicContactMap(Paras, ax):
    ...
    if 'mat' in Paras.keys():
        if Paras['mat']['method'] == 'extractCisContact':
            ...
            if 'row_regions' in Paras['mat'].keys() or 'col_regions' in Paras['mat'].keys():
                del Paras['mat']['method']
                mat_obj = tc.tl.extractContactRegions(clr=Paras['mat']['clr'], row_regions=region)
                mat = mat_obj.cmat
        else:
            pass
    ...
    if 'mat2' in Paras.keys():
        if Paras['mat2']['method'] == 'extractCisContact':
            ...
            if 'row_regions' in Paras['mat2'].keys() or 'col_regions' in Paras['mat2'].keys():
                del Paras['mat2']['method']
                mat_obj = tc.tl.extractContactRegions(clr=Paras['mat2']['clr'], row_regions=region)
                mat = mat_obj.cmat
        else:
            pass
...
def track(config, regions, outfile, basefigsize):
    ...
    regions_type = ['multi_scale_track', 'bed_track', 'bw_track', 'virtual4C', 'links_track', 'gene_track']
        ...
        if track_type=="hicmap":
            if 'mat' in paras.keys():
                if paras['mat']['method'] == 'extractCisContact':
                    ...
                    if 'row_regions' not in paras['mat'].keys() and 'col_regions' not in paras['mat'].keys():
            ...
            if 'mat2' in paras.keys():
                if paras['mat2']['method'] == 'extractCisContact':
                    ...
                    if 'row_regions' not in paras['mat2'].keys() and 'col_regions' not in paras['mat2'].keys():
