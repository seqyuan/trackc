...
class trackcConfig:
    ...
    def autosave(self) -> bool:
        """\
        Automatically save figures in :attr:`~trackc._settings.trackcConfig.figdir` (default `False`).
    ...
    def writedir(self) -> Path:
        """\
        Directory where the function trackc.write writes to by default.
        """
    ...
    def datasetdir(self) -> Path:
        """\
        Directory for example :mod:`~trackc.datasets` (default `'./data/'`).
        """
    ...
    def set_figure_params(
        ...
        ----------
        trackc
            Init default values for :obj:`matplotlib.rcParams` suited for trackc.
        dpi
        ...
        fontsize
            Set the fontsize for several `rcParams` entries. Ignored if `trackc=False`.
        figsize
            Set plt.rcParams['figure.figsize'].
        color_map
            Convenience method for setting the default color map. Ignored if `trackc=False`.
        format
        ...
        if facecolor is not None:
            ...
            rcParams['axes.facecolor'] = facecolor
        if trackc:
            from .plotting._rcmod import set_rcParams_trackc

            set_rcParams_trackc(fontsize=fontsize, color_map=color_map)
